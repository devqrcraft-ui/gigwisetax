...
export default function SCorpCalculator() {
  ...
  const btnR = { background:'#B22234', color:'#fff', border:'none', borderRadius:6, padding:'14px', fontSize:16, fontWeight:800, cursor:'pointer', width:'100%', marginTop:16 } as const
  ...
  const Row = ({ l, v, b }: { l:string; v:string; b?:boolean }) => (
    <div style={{ display:'flex', justifyContent:'space-between', padding:'9px 0', borderBottom:'1px solid #f3f4f6' }}>
      <span style={{ fontSize:14, color:b?'#1a1a2e':'#6b7280', fontWeight:b?700:400 }}>{l}</span>
      <span style={{ fontSize:14, fontWeight:b?800:600, color:b?'#1a1a2e':'#374151' }}>{v}</span>
    </div>
  )
  ...
  return (
    <div style={bg}><div style={wrap}>
      ...
      <div style={{ background:'rgba(232,184,75,.1)', border:'1px solid rgba(232,184,75,.3)', borderRadius:8, padding:'14px 18px', marginBottom:24, display:'flex', gap:12 }}>
        <span style={{ fontSize:20 }}>💡</span>
        <div style={{ fontSize:13, color:'rgba(255,255,255,.7)', lineHeight:1.6 }}>
          <strong style={{ color:'#e8b84b' }}>Did you know?</strong> Gig workers earning $60k+ can save $3,000–$12,000/year in self-employment taxes by electing S-Corp status.
        </div>
      </div>
      ...
      <div style={card}>
        ...
        <div style={body}>
          <label style={lbl}>Annual Net Profit (after expenses)</label>
          <input type="number" value={income} onChange={e => setIncome(e.target.value)} placeholder="e.g. 80000" style={inp}/>
          ...
          <label style={lbl}>Your Salary as S-Corp Owner <span style={{ color:'#9ca3af', fontWeight:400 }}>(optional)</span></label>
          <input type="number" value={salary} onChange={e => setSalary(e.target.value)} placeholder="e.g. 50000 (IRS requires reasonable salary)" style={inp}/>
          <p style={{ fontSize:12, color:'#9ca3af', marginTop:6 }}>Typically 40–60% of profit. The rest comes as distributions — not subject to SE tax.</p>
          <button onClick={() => income && calculate()} style={{ ...btnR, opacity: income ? 1 : 0.5 }}>Calculate My S-Corp Savings →</button>
        </div>
      </div>
        ...
        <div style={{ ...card, border: result.savings > 0 ? '2px solid #22c55e' : '2px solid #e5e7eb' }}>
          <div style={{ background: result.savings > 0 ? '#f0fdf4' : '#f9fafb', padding:'24px', textAlign:'center' }}>
            {result.savings > 0 ? (<>
              ...
              <div style={{ fontSize:13, color:'#374151', marginTop:12, fontWeight:600 }}>Over 5 years: <span style={{ color:'#16a34a', fontWeight:800 }}>{fmt(result.savings * 5)}</span></div>
            </>) : (<>
          ...
          <div style={body}>
            ...
            {result.savings > 0 && (
              <div style={{ background:'#1e2d5a', borderRadius:8, padding:16, marginBottom:16 }}>
                ...
                {['Form an LLC (if not already) — ~$100-500 depending on state','File IRS Form 2553 (Election by a Small Business Corporation)','Set up payroll for your salary (use Gusto or ADP)','File Form 1120-S annually (use a CPA — ~$1,500-2,500/yr)','Distribute remaining profit as dividends — no SE tax!'].map((s,i) => (
                  <div key={i} style={{ display:'flex', gap:10, marginBottom:7, alignItems:'flex-start' }}>
                    <span style={{ background:'#B22234', color:'#fff', borderRadius:'50%', width:18, height:18, display:'flex', alignItems:'center', justifyContent:'center', fontSize:10, fontWeight:800, flexShrink:0 }}>{i+1}</span>
      ...
      <p style={{ color:'rgba(255,255,255,.2)', fontSize:11, textAlign:'center', marginTop:16 }}>For informational purposes only. Consult a licensed CPA. GigWiseTax.com</p>
    </div></div>
  )
}
