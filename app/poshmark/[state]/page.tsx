...
export async function generateMetadata({ params }: { params: Promise<{ state: string }> }): Promise<Metadata> {
  ...
  return {
    title: s.rate === 0
      ...
      : `Poshmark ${s.name} Taxes 2026 — $60K Seller Owes $${(3888 + Math.round(23000 * s.rate) - Math.round(23000 * 0.05)).toLocaleString()} Total`,
    description: `Poshmark ${s.name} 2026: $60K gross, $25K COGS = $23K net profit = ~$3,250 SE tax + $638 fed. ${stateStr}. Free calculator with COGS, no signup.`,
...
export default async function StatePage({ params }: { params: Promise<{ state: string }> }) {
  ...
  const stateNote = state.name + (noStateTax ? ' has no state income tax. Poshmark resellers in ' + state.name + ' only pay federal income tax and the 15.3% self-employment tax on net profit.' : ' has a ' + stateRateStr + ' state income tax rate. Poshmark resellers in ' + state.name + ' must make both federal and state quarterly estimated tax payments on net profit.')
  ...
  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: '{"@context":"https://schema.org","@type":"FAQPage","mainEntity":[{"@type":"Question","name":"Do Poshmark sellers pay taxes?","acceptedAnswer":{"@type":"Answer","text":"Yes, on net profit. Sellers pay self-employment tax (15.3%) plus federal and state income tax on sale price minus Poshmark commission minus cost of goods sold, once net profit exceeds $400."}},{"@type":"Question","name":"Does Poshmark send a 1099-K?","acceptedAnswer":{"@type":"Answer","text":"Poshmark issues a 1099-K if gross sales exceed $20,000 and you have more than 200 transactions in a year, the federal threshold restored by the 2025 OBBBA law. Some states require it at much lower thresholds. Even without a 1099-K, all income is taxable."}},{"@type":"Question","name":"What can Poshmark sellers deduct?","acceptedAnswer":{"@type":"Answer","text":"Poshmark sellers can deduct cost of goods sold (what they paid for inventory), Poshmark commission, shipping supplies, mileage to source inventory, a home office if used exclusively for the business, and photography equipment."}}]}' }} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: '{"@context":"https://schema.org","@type":"WebApplication","name":"Poshmark Tax Calculator ' + state.name + ' 2026","applicationCategory":"FinanceApplication","offers":{"@type":"Offer","price":"0","priceCurrency":"USD"}}' }} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"Home","item":"https://www.gigwisetax.com"},{"@type":"ListItem","position":2,"name":"Poshmark Tax Calculator","item":"https://www.gigwisetax.com/poshmark"},{"@type":"ListItem","position":3,"name":"' + state.name + '","item":"https://www.gigwisetax.com/poshmark/' + state.slug + '"}]}' }} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: '{"@context":"https://schema.org","@type":"WebPage","name":"Poshmark Tax Calculator ' + state.name + ' 2026","url":"https://www.gigwisetax.com/poshmark/' + state.slug + '","dateModified":"2026-07-27","author":{"@type":"Person","name":"Ethan Blake","url":"https://medium.com/@dev.qrcraft"},"publisher":{"@type":"Organization","name":"GigWiseTax","url":"https://www.gigwisetax.com"}}' }} />
        ...
        <div style={{ background: 'linear-gradient(135deg,#1e2d5a,#07111F)', borderBottom: '1px solid rgba(255,255,255,.07)' }}>
          <div style={{ maxWidth: 1200, margin: '0 auto', padding: '16px 16px 12px' }}>
            ...
            <p style={{ color: 'rgba(255,255,255,.55)', fontSize: 14, lineHeight: 1.8, paddingLeft: 16, maxWidth: 780, marginBottom: 18 }}>
              {'Free ' + platform.name + ' reseller tax calculator for ' + state.name + ' (' + state.abbr + '), including cost of goods sold.'}
        ...
        <div style={{ maxWidth:1200, margin:'0 auto', padding:'4px 16px 0' }}>
          ...
          <section id="answer-first" style={{ background:'rgba(232,184,75,0.06)', border:'1px solid rgba(232,184,75,0.2)', borderRadius:8, padding:'20px 24px', marginBottom:24 }}>
            ...
            <p style={{ fontSize:14, color:'rgba(255,255,255,0.85)', lineHeight:1.8, margin:0 }}>
              {'Poshmark resellers in ' + state.name + ' with $60,000 gross sales and $25,000 in cost of goods sold owe approximately $' + (3888 + Math.round(23000 * state.rate) - Math.round(23000 * 0.05)).toLocaleString() + ' total — net profit of $23,000, after Poshmark\'s 20% commission and cost of goods, taxed at $3,250 self-employment tax (15.3%) + $638 federal income tax' + (noStateTax ? '. ' + state.name + ' has no state income tax.' : ' + ' + stateRateStr + ' ' + state.name + ' state income tax.') + ' Quarterly estimated payment: ~$' + Math.round((3888 + Math.round(23000 * state.rate) - Math.round(23000 * 0.05)) / 4).toLocaleString() + '. The 1099-K reports gross sales, not this profit figure.'}
            </p>
          </section>
          <section id="key-takeaways" style={{ background:'rgba(232,184,75,0.06)', border:'1px solid rgba(232,184,75,0.2)', borderRadius:8, padding:'20px 24px', marginBottom:24 }}>
            ...
            <ul style={{ margin:0, padding:'0 0 0 18px', lineHeight:1.9, fontSize:14, color:'#C8D8EC' }}>
              <li>Poshmark resellers are independent contractors — Poshmark withholds zero taxes</li>
              <li>{'On $60,000 gross sales with $25,000 COGS: net profit $23,000 → ~$3,250 SE tax + ~$638 federal = ~$3,888 total in ' + state.name}</li>
              <li>{'Quarterly payments due Apr 15, Jun 16, Sep 15, Jan 15 — ~$972 each'}</li>
              <li>The 1099-K reports gross sales — Poshmark commission and cost of goods come off before you owe tax</li>
              <li>Federal 1099-K threshold: $20,000 and 200+ transactions — some states set it far lower</li>
              <li>File Schedule C — report all Poshmark income even without a 1099-K</li>
            </ul>
          </section>
        </div>
            ...
            <div style={card}>
              ...
              <div style={{ padding: 14 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 12 }} className="three-grid">
                  {[
                    { label: 'SE Tax', val: '15.3%', sub: 'On net profit, federal', color: '#B22234' },
                    { label: 'Federal Tax', val: '10-37%', sub: 'Based on net profit', color: 'rgba(255,255,255,0.9)' },
                ...
                <h3 style={{ fontSize: 17, fontWeight: 800, color: 'rgba(255,255,255,0.9)', marginBottom: 12 }}>Top Tax Deductions for {platform.name} Sellers in {state.name}</h3>
                ...
                <blockquote style={{ borderLeft:'3px solid rgba(232,184,75,0.5)', margin:'16px 0', padding:'12px 20px', background:'rgba(232,184,75,0.04)', borderRadius:'0 6px 6px 0' }}>
                  <p style={{ margin:0, fontSize:13, fontStyle:'italic', color:'rgba(255,255,255,0.7)', lineHeight:1.7 }}>
                    {'The 1099-K reports gross sales before Poshmark\'s commission and before cost of goods sold — it is not your taxable profit.'} — <a href="https://www.irs.gov/businesses/small-businesses-self-employed/self-employed-individuals-tax-center" rel="nofollow" target="_blank" style={{ color:'#e8b84b' }}>IRS Self-Employed Tax Center</a>
                  </p>
                </blockquote>
                ...
                {[
                  { q: 'Do Poshmark sellers pay taxes in ' + state.name + '?', a: noStateTax ? 'Yes, on net profit. In ' + state.name + ', Poshmark sellers pay 15.3% self-employment tax plus federal income tax. There is no ' + state.name + ' state income tax.' : 'Yes, on net profit. In ' + state.name + ', Poshmark sellers pay 15.3% SE tax, federal income tax (10-37%), plus ' + stateRateStr + ' ' + state.name + ' state income tax.' },
                  ...
                  { q: 'Does the 1099-K show my Poshmark profit in ' + state.name + '?', a: 'No. The 1099-K reports gross sales — what buyers paid before Poshmark\'s 20% commission and before your cost of goods sold. Your actual taxable profit is smaller, and it is the same calculation' + (noStateTax ? '' : ' plus ' + state.name + ' state tax') + ' regardless of which state you live in.' },
                ].map((item, i) => (
                ...
                <div style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 4, padding: 12, fontSize: 14, color: '#8fa8c8', lineHeight: 1.6 }}>
                  <strong>Disclaimer:</strong> State tax rates are for estimation purposes only. Not affiliated with Poshmark or any government agency. Consult a licensed CPA for advice.
                </div>
              </div>
            </div>
            <div style={card}>
