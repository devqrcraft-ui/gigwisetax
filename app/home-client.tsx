                ...
                <div style={card}>
                  ...
                  <div style={{ overflowX: 'auto' as const }}><div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(4,1fr)', borderBottom: '1px solid #e2e5e9', minWidth: 0 }} className="results-row">
                    {[
                      ...
                      <div key={r.label} style={{ padding: isMobile ? '14px 16px' : '16px 14px', borderRight: isMobile ? 'none' : (i < 3 ? '1px solid #e2e5e9' : 'none'), borderBottom: isMobile ? '1px solid #e2e5e9' : 'none', background: r.hi ? '#fff5f5' : '#f8fafc', borderLeft: r.hi ? '4px solid #B22234' : 'none', borderRadius: isMobile ? 6 : 0 }}>
                  ...
                  {/* HIGH INCOME SECTION */}
                  {result && parseFloat(income) >= 100000 && (
                    <div style={{ marginTop: 14, background: 'linear-gradient(135deg,rgba(178,34,52,0.08),rgba(26,26,46,0.5))', border: '1px solid rgba(178,34,52,0.3)', borderRadius: 8, padding: '16px 18px' }}>
                      <div style={{ fontSize: 13, fontWeight: 800, color: '#B22234', marginBottom: 10, textTransform: 'uppercase' as const, letterSpacing: '0.5px' }}>⚠️ High-Income Gig Worker Alert</div>
                      <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 8 }}>
                        {[
                          parseFloat(income) >= 168600 ? '📊 SE Tax Social Security cap: $168,600 in 2026 — earnings above this are exempt from 12.4% SS tax, saving up to $20,905.' : null,
                          parseFloat(income) >= 200000 ? '💊 Additional Medicare surtax: 0.9% on income above $200,000 (single) — adds up to $2,700+ on a $500k income.' : null,
                          '🗺️ State top brackets: CA hits 13.3% above $1M, NY 10.9% above $25M — high earners in these states face combined marginal rates of 50%+.',
                          '💼 Consider S-Corp election: At $80k+ net profit, an S-Corp salary split can cut SE tax by $5,000–$15,000/year.',
                        ].filter(Boolean).map((bullet, i) => (
                          <div key={i} style={{ fontSize: 12, color: 'rgba(255,255,255,0.75)', lineHeight: 1.6, paddingLeft: 4 }}>{bullet}</div>
                        ))}
                      </div>
                      <a href="/s-corp-calculator" style={{ display: 'inline-block', marginTop: 12, fontSize: 12, fontWeight: 700, color: '#e8b84b', textDecoration: 'none' }}>→ S-Corp Tax Savings Calculator</a>
                    </div>
                  )}
                    ...
                    <thead>
                      <tr style={{ background: '#f8fafc', borderBottom: '2px solid #e2e5e9' }}>
                        {['Feature','GigWiseTax ✓','FlyFin','Everlance','TurboTax Blog'].map((h,i) => (
                          <th key={h} style={{ padding: '10px 8px', fontWeight: 700, fontSize: 11, color: i===1 ? '#B22234' : '#6c757d', textAlign: i===0 ? 'left' as const : 'center' as const, background: i===1 ? '#fff5f5' : 'transparent', borderBottom: i===1 ? '3px solid #B22234' : 'none', borderTop: i===1 ? '3px solid #B22234' : 'none', boxShadow: i===1 ? 'inset 2px 0 0 #B22234, inset -2px 0 0 #B22234' : 'none' }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {[
                        ...
                        <tr key={feat as string} style={{ borderBottom: '1px solid #f0f1f3' }}>
                          ...
                          {(vals as boolean[]).map((v,i) => (
                            <td key={i} style={{ padding: '8px 6px', textAlign: 'center' as const, background: i===0 ? 'rgba(178,34,52,0.06)' : 'transparent', borderLeft: i===0 ? '2px solid rgba(178,34,52,0.2)' : 'none', borderRight: i===0 ? '2px solid rgba(178,34,52,0.2)' : 'none' }}>
                              <span style={{ fontSize: 15, fontWeight: 800, color: v ? (i===0 ? '#B22234' : '#059669') : '#d1d5db' }}>{v ? '✓' : '✗'}</span>
                            </td>
                          ))}
                        </tr>
                      ))}
