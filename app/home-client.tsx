...
export default function HomeClient() {
  ...
  return (
    <div style={{ background: '#07111F', minHeight: '100vh', overflowX: 'hidden' as const, maxWidth: '100vw' }}>
      ...
      {/* ━━ MAIN GRID ━━ */}
      <div className="main-grid-outer" style={{ maxWidth: 1200, margin: '0 auto', padding: '8px 12px 20px', display: 'grid', gridTemplateColumns: '1fr', gap: 14 }}>

        {/* ── LEFT ── */}
        <div>

          {/* TABS */}
          <div style={{ display: 'flex', borderBottom: '2px solid #d8dce6', marginBottom: 12, gap: 6, padding: '4px 4px 0' }}>
            {([
              ...
              <div key={t.id} onClick={() => setTab(t.id)} style={{
                ...
                {t.label}{tab !== t.id ? <span style={{marginLeft:8,fontSize:16,opacity:0.9,fontWeight:700}}>→</span> : <span style={{marginLeft:6,fontSize:14,fontWeight:700}}>▼</span>}
              </div>
            ))}
          </div>
            ...
            <>
              ...
              <div style={card}>
                ...
                <div style={{ padding: 16 }}>
                  ...
                  {/* Section 1 */}
                  <div style={{ fontSize: 13, fontWeight: 800, color: '#8fa8c8', textTransform: 'uppercase' as const, letterSpacing: '0.8px', marginBottom: 8, paddingBottom: 6, borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                    1. Your Gig Income
                  </div>
            ...
            {['20000','50000','100000','250000','500000'].map(v=>(
              <button key={v} onClick={()=>setIncome(v)} style={{padding:'4px 10px',borderRadius:'20px',border:'1px solid rgba(232,184,75,0.4)',background:income===v?'#e8b84b':'rgba(30,45,70,0.8)',color:income===v?'#07111f':'rgba(255,255,255,0.85)',fontSize:'12px',fontWeight:600,cursor:'pointer'}}>
                {v==='20000'?'$20k':v==='50000'?'$50k':v==='100000'?'$100k':v==='250000'?'$250k':'$500k'}
                  ...
                  {/* Section 2 */}
                  <div style={{ fontSize: 13, fontWeight: 800, color: '#8fa8c8', textTransform: 'uppercase' as const, letterSpacing: '0.8px', marginBottom: 8, paddingBottom: 6, borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                    2. Business Expenses <span style={{ fontWeight: 400, fontSize: 12, color: '#7a9abf' }}>(optional)</span>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 12 }} className="form-grid">
                    <div>
                    ...
                    <div style={{ overflowX: 'auto' as const }}><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, minWidth: 0 }} className="q-grid">
                      {DEADLINES.map((d, i) => (
                        <div key={d.q} style={{ border: i === 0 ? '2px solid #e8b84b' : '1px solid rgba(255,255,255,0.1)', borderRadius: 6, padding: 10, background: i === 0 ? 'rgba(232,184,75,0.12)' : 'rgba(255,255,255,0.04)', position: 'relative' as const }}>
                          ...
                          <div style={{...btnRed({ fontSize: 13, padding: '6px 0' }), cursor:'pointer'}} onClick={()=>{ const urls = {'Q1 2026':'https://calendar.google.com/calendar/render?action=TEMPLATE&text=Q1%202026%20IRS%20Estimated%20Tax%20Payment%20Due&dates=20260415/20260416&details=IRS%20quarterly%20estimated%20tax%20payment%20due.%20Pay%20at%3A%20https%3A%2F%2Fwww.irs.gov%2Fpayments&sf=true&output=xml','Q2 2026':'https://calendar.google.com/calendar/render?action=TEMPLATE&text=Q2%202026%20IRS%20Estimated%20Tax%20Payment%20Due&dates=20260616/20260617&details=IRS%20quarterly%20estimated%20tax%20payment%20due.%20Pay%20at%3A%20https%3A%2F%2Fwww.irs.gov%2Fpayments&sf=true&output=xml','Q3 2026':'https://calendar.google.com/calendar/render?action=TEMPLATE&text=Q3%202026%20IRS%20Estimated%20Tax%20Payment%20Due&dates=20260915/20260916&details=IRS%20quarterly%20estimated%20tax%20payment%20due.%20Pay%20at%3A%20https%3A%2F%2Fwww.irs.gov%2Fpayments&sf=true&output=xml','Q4 2026':'https://calendar.google.com/calendar/render?action=TEMPLATE&text=Q4%202026%20IRS%20Estimated%20Tax%20Payment%20Due&dates=20270115/20270116&details=IRS%20quarterly%20estimated%20tax%20payment%20due.%20Pay%20at%3A%20https%3A%2F%2Fwww.irs.gov%2Fpayments&sf=true&output=xml'}; window.open(urls[d.q],'_blank'); }}>+ Calendar</div>
                        </div>
                      ))}
                    </div></div>
              ...
              {result && parseFloat(income) >= 150000 && (
                <div style={{ background: 'linear-gradient(135deg,#1a0a2e,#2d1454)', border: '1px solid rgba(168,85,247,0.4)', borderRadius: 8, padding: '18px 20px', marginBottom: 10 }}>
            ...
            <div style={card}>
              ...
              <div style={{ overflowX: 'auto' as const }}>
                <div style={{overflowX:'auto',WebkitOverflowScrolling:'touch'}}><table style={{ width: '100%', borderCollapse: 'collapse' as const }}>
                  ...
                  <tbody>
                    {PLATFORMS.map((p,i) => (
                      <tr key={p.name} onClick={()=>window.location.href=p.href} style={{ borderBottom: '1px solid rgba(255,255,255,0.08)', background: i%2===0 ? 'rgba(255,255,255,0.04)' : 'rgba(255,255,255,0.01)', cursor:'pointer', transition:'background .15s' }} onMouseEnter={e=>(e.currentTarget.style.background='rgba(232,184,75,0.08)')} onMouseLeave={e=>(e.currentTarget.style.background=i%2===0?'rgba(255,255,255,0.04)':'rgba(255,255,255,0.01)')}>
                        ...
                        <td style={{ padding: '12px 16px', color: 'rgba(255,255,255,0.07)', fontWeight: 600 }}>15.3%</td>
                        <td style={{ padding: '12px 16px', color: '#059669', fontWeight: 600 }}>Included ✓</td>
                        <td style={{ padding: '12px 16px' }}>
                          <span style={{ background: 'rgba(5,150,105,0.2)', color: '#4ade80', padding: '3px 10px', borderRadius: 12, fontSize: 13, fontWeight: 700, border: '1px solid rgba(74,222,128,0.3)' }}>Low ✓</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 16 }}>
          ...
          <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 6, padding: 16 }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: '#8fa8c8', textTransform: 'uppercase' as const, letterSpacing: '1px', marginBottom: 10 }}> About This Tool</div>
            ...
            <div style={{ marginTop: 12, display: 'flex', gap: 12 }}>
              {['Privacy','Terms','Contact'].map(l => (
                <span key={l} style={{ fontSize: 13, color: '#8fa8c8', cursor: 'pointer', textDecoration: 'underline' }}>{l}</span>
              ))}
            </div>
          </div>
        ...
        <div style={{marginBottom:48,maxWidth:800}}>
          ...
          <div style={{display:'flex',flexDirection:'column' as const,gap:8}}>
            {[
              ...
              ['Can I deduct mileage as a DoorDash or Uber driver?','Yes — the 2026 IRS standard mileage rate is 72.5¢/mile through June 30, 72.5¢/mile from July 1 onward. A driver logging 15,000 business miles saves roughly $11,000-$11,400 in deductible expenses, which reduces SE tax and income tax.'],
