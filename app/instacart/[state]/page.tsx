...
export async function generateMetadata({ params }: { params: Promise<{ state: string }> }): Promise<Metadata> {
  ...
  return {
    title: s.rate === 0
      ? `Instacart ${s.name} Taxes 2026 — No State Tax + Free SE Calculator`
      : `Instacart ${s.name} Taxes 2026 — $35K Earner Owes $${(7750 + Math.round(35000 * s.rate)).toLocaleString()} Total`,
    description: `Instacart ${s.name} 2026: $35K net = ~$4,944 SE tax + $2,800 fed. ${stateStr}. Quarterly: ~$1,938. Free calculator, no signup.`,
...
export default async function StatePage({ params }: { params: Promise<{ state: string }> }) {
  ...
  return (
    <>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: '{"@context":"https://schema.org","@type":"FAQPage","mainEntity":[{"@type":"Question","name":"How much do Instacart shoppers pay in taxes?","acceptedAnswer":{"@type":"Answer","text":"Instacart full-service shoppers pay self-employment tax (15.3%) plus income tax as independent contractors. In-store shoppers are W-2 employees with taxes withheld. Full-service shoppers should save 25-30% of earnings."}},{"@type":"Question","name":"Does Instacart withhold taxes from shoppers?","acceptedAnswer":{"@type":"Answer","text":"Only for in-store part-time employees. Full-service Instacart shoppers are 1099 contractors — no taxes are withheld. You must make quarterly estimated payments to avoid IRS penalties."}},{"@type":"Question","name":"Can Instacart shoppers deduct mileage?","acceptedAnswer":{"@type":"Answer","text":"Yes. Full-service Instacart shoppers can deduct mileage at 72.5 cents/mile (2026) from the store to the customer. Mileage from home to the store is not deductible unless you have a home office."}}]}' }} />
                ...
                {[
                  {
                    ...
                    a: noStateTax
                      ...
                      : `In ${state.name}, ${platform.name} workers pay 15.3% self-employment tax, federal income tax (10–37%), plus ${stateRateStr} ${state.name} state income tax. On $35,000 net income, expect to owe approximately $${(7750 + Math.round(35000 * state.rate)).toLocaleString()} total.`,
                  },
                  {
