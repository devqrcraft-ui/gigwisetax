...
export default async function StatePage({ params }: { params: Promise<{ state: string }> }) {
  ...
  return (
    <>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: '{"@context":"https://schema.org","@type":"FAQPage","mainEntity":[{"@type":"Question","name":"How much tax do Uber drivers pay?","acceptedAnswer":{"@type":"Answer","text":"Uber drivers pay self-employment tax of 15.3% on net earnings plus federal income tax based on their bracket. Most drivers pay an effective total tax rate of 25-35% on net profit after deductions."}},{"@type":"Question","name":"Does Uber take out taxes?","acceptedAnswer":{"@type":"Answer","text":"No. Uber does not withhold taxes for drivers. Drivers are independent contractors who receive a 1099-K or 1099-NEC and must pay estimated quarterly taxes to the IRS."}},{"@type":"Question","name":"What is the best tax deduction for Uber drivers?","acceptedAnswer":{"@type":"Answer","text":"The mileage deduction at 67 cents/mile (2026 IRS rate) is the largest deduction for most Uber drivers. On 30,000 miles driven, that is a $20,100 deduction that directly reduces taxable income."}}]}' }} />
              ...
              <div style={{ padding: 24 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 20 }} className="form-grid">
                  {deductions.map((d: string) => (
                    <div key={d} style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid #e2e5e9', borderRadius: 4, padding: '10px 14px', fontSize: 13, color: 'rgba(255,255,255,0.85)', fontWeight: 500 }}>{d}</div>
                  ))}
                </div>
              ...
              <div style={{ padding: 16, display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8 }} className="p-grid">
                {STATES.filter(s => s.slug !== stateSlug).map(s => (
                  <a key={s.slug} href={`/${PLATFORM_SLUG}/${s.slug}`} style={{ textDecoration: 'none' }}>
                    <div style={{ border: '1px solid #e2e5e9', borderRadius: 4, padding: '8px 10px', textAlign: 'center' as const, background: 'rgba(255,255,255,0.03)' }}
                    >
