...
export async function generateMetadata({ params }: { params: { platform: string } }): Promise<Metadata> {
  ...
  return {
    ...
    keywords: `taskrabbit tax calculator, taskrabbit self employment tax 2026, taskrabbit quarterly taxes, taskrabbit 1099 taxes, how much taxes do taskrabbit workers pay`,
...
export default function PlatformPage({ params }: { params: { platform: string } }) {
  ...
  const deductions = DEDUCTIONS['taskrabbit' as keyof typeof DEDUCTIONS] || DEDUCTIONS.taskrabbit
  ...
  return (
    <>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: `{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"Home","item":"https://www.gigwisetax.com"},{"@type":"ListItem","position":2,"name":"${platform.name} Tax Calculator","item":"https://www.gigwisetax.com/taskrabbit"}]}` }}/>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: `{"@context":"https://schema.org","@type":"WebPage","name":"${platform.name} Tax Calculator 2026","url":"https://www.gigwisetax.com/taskrabbit","dateModified":"2026-08-24","author":{"@type":"Organization","name":"the GigWiseTax Team","url":"https://www.gigwisetax.com"},"publisher":{"@type":"Organization","name":"GigWiseTax","url":"https://www.gigwisetax.com"}}` }}/>
      ...
      <div style={{ background: '#07111F', minHeight: '100vh' }}>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '24px 20px 48px', display: 'grid', gridTemplateColumns: '1fr 300px', gap: 24 }} className="main-grid">
          ...
          <li dangerouslySetInnerHTML={{__html: 'IRS mileage rate is <strong>72.5¢/mile through June, 72.5¢/mile from July 2026</strong> — one of the largest deductions available'}}/>
          ...
          <div style={{ background:'rgba(232,184,75,0.06)', border:'1px solid rgba(232,184,75,0.2)', borderRadius:8, padding:'20px 24px', margin:'16px 0' }}>
            ...
            <div style={card}>
              ...
              <div style={{ padding: 16, display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10 }} className="p-grid">
                {PLATFORMS.filter(p => p.slug !== 'taskrabbit').map(p => (
...
<div style={{ background: '#07111F', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 10, overflow: 'hidden' as const, marginBottom: 16 }}>
          ...
          {/* OWN BANNER — privatepaycheck.com */}
            ...
            {/* STATES LINKS */}
            <PlatformStatesGrid platformSlug="taskrabbit" />
...
{/* RELATED ARTICLES */}
      <section style={{ maxWidth: 1200, margin: '0 auto', padding: '0 20px 24px' }}>
        ...
        <ul style={{ paddingLeft: 20, lineHeight: 2, fontSize: 14 }}>
            <li><a href="/blog/taskrabbit-taxes-2026" style={{ color: '#e8b84b', textDecoration: 'none' }}>TaskRabbit Taxes 2026: Complete Guide</a></li>
        </ul>
      </section>
      <AuthorBox />
      <style>{`
