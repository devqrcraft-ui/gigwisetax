...
const PLATFORM_SLUG = 'taskrabbit'
...
export async function generateMetadata({ params }: { params: Promise<{ state: string }> }): Promise<Metadata> {
  ...
  return {
    title: s.rate === 0
      ...
      : `TaskRabbit ${s.name} Taxes 2026 — $40K Earner Owes $${(8129 + Math.round(40000 * s.rate)).toLocaleString()} Total`,
    description: `TaskRabbit ${s.name} 2026: $45K net = ~$6,360 SE tax + $3,800 fed. ${stateStr}. Quarterly: ~$2,540. Free calculator, no signup.`,
...
export default async function StatePage({ params }: { params: Promise<{ state: string }> }) {
  ...
  const stateNote = state.name + (noStateTax ? ' has no state income tax. TaskRabbit workers in ' + state.name + ' only pay federal income tax and the 15.3% self-employment tax.' : ' has a ' + stateRateStr + ' state income tax rate. TaskRabbit workers in ' + state.name + ' must make both federal and state quarterly estimated tax payments.')
  ...
  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: '{"@context":"https://schema.org","@type":"FAQPage","mainEntity":[{"@type":"Question","name":"Do TaskRabbit workers pay taxes?","acceptedAnswer":{"@type":"Answer","text":"Yes. TaskRabbit classifies taskers as independent contractors. TaskRabbit does not withhold taxes. If you earn $400 or more you must pay self-employment tax (15.3%) plus federal and state income tax."}},{"@type":"Question","name":"Does TaskRabbit send a 1099?","acceptedAnswer":{"@type":"Answer","text":"TaskRabbit sends a 1099-K if you process $5,000 or more through the platform in a year. Even without a 1099 all income is taxable and must be reported on Schedule C."}},{"@type":"Question","name":"What can TaskRabbit workers deduct?","acceptedAnswer":{"@type":"Answer","text":"TaskRabbit workers can deduct tools and equipment, work supplies, mileage driving to client locations (72.5 cents/mile in 2026), a home office if used exclusively for the business, and any training costs."}}]}' }} />
          ...
          <div>
            ...
            <div style={card}>
              ...
              <div style={{ padding: 14 }}>
                ...
                {[
                  { q: 'Do TaskRabbit workers pay taxes in ' + state.name + '?', a: noStateTax ? 'Yes. In ' + state.name + ', TaskRabbit taskers pay 15.3% self-employment tax plus federal income tax. There is no ' + state.name + ' state income tax.' : 'Yes. In ' + state.name + ', TaskRabbit taskers pay 15.3% SE tax, federal income tax (10-37%), plus ' + stateRateStr + ' ' + state.name + ' state income tax.' },
                  ...
                  { q: 'Does TaskRabbit take out taxes in ' + state.name + '?', a: 'No. TaskRabbit does not withhold taxes. You are responsible for paying all SE tax, federal income tax' + (noStateTax ? '' : ', and ' + state.name + ' state income tax') + ' yourself through quarterly estimated payments.' },
                ].map((item, i) => (
                ...
                <div style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 4, padding: 12, fontSize: 14, color: '#8fa8c8', lineHeight: 1.6 }}>
                  <strong>Disclaimer:</strong> State tax rates are for estimation purposes only. Not affiliated with TaskRabbit or any government agency. Consult a licensed CPA for advice.
                </div>
              </div>
            </div>
            <div style={card}>
