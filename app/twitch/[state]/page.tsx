...
export async function generateMetadata({ params }: { params: Promise<{ state: string }> }): Promise<Metadata> {
  ...
  return {
    title: s.rate === 0
      ...
      : `Twitch ${s.name} Taxes 2026 — $30K Streamer Owes $${(5546 + Math.round(30000 * s.rate)).toLocaleString()} Total`,
...
export default async function StatePage({ params }: { params: Promise<{ state: string }> }) {
  ...
  const stateNote = state.name + (noStateTax ? ' has no state income tax. Twitch streamers in ' + state.name + ' only pay federal income tax and the 15.3% self-employment tax.' : ' has a ' + stateRateStr + ' state income tax rate. Twitch streamers in ' + state.name + ' must make both federal and state quarterly estimated tax payments.')
  ...
  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: '{"@context":"https://schema.org","@type":"FAQPage","mainEntity":[{"@type":"Question","name":"Do Twitch streamers pay taxes?","acceptedAnswer":{"@type":"Answer","text":"Yes. Twitch classifies streamers as independent contractors. Twitch does not withhold taxes. If you earn $400 or more you must pay self-employment tax (15.3%) plus federal and state income tax."}},{"@type":"Question","name":"Why did I get two 1099 forms from Twitch?","acceptedAnswer":{"@type":"Answer","text":"Twitch reports subscription and ad revenue on Form 1099-MISC (Box 2, as royalties), while bits are reported separately on Form 1099-NEC. Both count as self-employment income and must be combined on Schedule C."}},{"@type":"Question","name":"What can Twitch streamers deduct?","acceptedAnswer":{"@type":"Answer","text":"Twitch streamers can deduct streaming equipment (camera, microphone, capture card), software subscriptions, a percentage of internet costs, a home office if used exclusively for streaming, and editing or graphic design software."}}]}' }} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: '{"@context":"https://schema.org","@type":"WebApplication","name":"Twitch Tax Calculator ' + state.name + ' 2026","applicationCategory":"FinanceApplication","offers":{"@type":"Offer","price":"0","priceCurrency":"USD"}}' }} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"Home","item":"https://www.gigwisetax.com"},{"@type":"ListItem","position":2,"name":"Twitch Tax Calculator","item":"https://www.gigwisetax.com/twitch"},{"@type":"ListItem","position":3,"name":"' + state.name + '","item":"https://www.gigwisetax.com/twitch/' + state.slug + '"}]}' }} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: '{"@context":"https://schema.org","@type":"WebPage","name":"Twitch Tax Calculator ' + state.name + ' 2026","url":"https://www.gigwisetax.com/twitch/' + state.slug + '","dateModified":"2026-07-28","author":{"@type":"Person","name":"Ethan Blake","url":"https://medium.com/@dev.qrcraft"},"publisher":{"@type":"Organization","name":"GigWiseTax","url":"https://www.gigwisetax.com"}}' }} />
        ...
        <div style={{ background: 'linear-gradient(135deg,#1e2d5a,#07111F)', borderBottom: '1px solid rgba(255,255,255,.07)' }}>
          <div style={{ maxWidth: 1200, margin: '0 auto', padding: '16px 16px 12px' }}>
            ...
            <p style={{ color: 'rgba(255,255,255,.55)', fontSize: 14, lineHeight: 1.8, paddingLeft: 16, maxWidth: 780, marginBottom: 18 }}>
              {'Free ' + platform.name + ' streamer tax calculator for ' + state.name + ' (' + state.abbr + ').'}
        ...
        <div style={{ maxWidth:1200, margin:'0 auto', padding:'4px 16px 0' }}>
          ...
          <section id="answer-first" style={{ background:'rgba(232,184,75,0.06)', border:'1px solid rgba(232,184,75,0.2)', borderRadius:8, padding:'20px 24px', marginBottom:24 }}>
            ...
            <p style={{ fontSize:14, color:'rgba(255,255,255,0.85)', lineHeight:1.8, margin:0 }}>
              {'Twitch streamers in ' + state.name + ' earning $30,000 net (combined subs, ads, bits, and donations) owe approximately $' + (5546 + Math.round(30000 * state.rate)).toLocaleString() + ' total — $4,239 self-employment tax (15.3%) + $1,307 federal income tax' + (noStateTax ? '. ' + state.name + ' has no state income tax.' : ' + ' + stateRateStr + ' ' + state.name + ' state income tax.') + ' Quarterly estimated payment: ~$' + Math.round((5546 + Math.round(30000 * state.rate)) / 4).toLocaleString() + '. Subscription and ad revenue arrives on a 1099-MISC, while bits arrive on a separate 1099-NEC — both count as self-employment income.'}
            </p>
          </section>
          <section id="key-takeaways" style={{ background:'rgba(232,184,75,0.06)', border:'1px solid rgba(232,184,75,0.2)', borderRadius:8, padding:'20px 24px', marginBottom:24 }}>
            ...
            <ul style={{ margin:0, padding:'0 0 0 18px', lineHeight:1.9, fontSize:14, color:'#C8D8EC' }}>
              <li>Twitch streamers are independent contractors — Twitch withholds zero taxes</li>
              <li>{'On $30,000 net income: ~$4,239 SE tax + ~$1,307 federal = ~$5,546 total in ' + state.name}</li>
              <li>{'Quarterly payments due Apr 15, Jun 16, Sep 15, Jan 15 — ~$1,387 each'}</li>
              <li>Twitch sends two separate 1099 forms — 1099-MISC for subs/ads, 1099-NEC for bits — both must be combined</li>
              <li>Streaming equipment, software, and internet percentage are deductible business expenses</li>
              <li>File Schedule C — report all Twitch income from both 1099 forms, even without either form</li>
            </ul>
          </section>
        </div>
          ...
          <div>
            ...
            <div style={card}>
              ...
              <div style={{ padding: 14 }}>
                ...
                <h3 style={{ fontSize: 17, fontWeight: 800, color: 'rgba(255,255,255,0.9)', marginBottom: 12 }}>Top Tax Deductions for {platform.name} Streamers in {state.name}</h3>
                ...
                {[
                  { q: 'Do Twitch streamers pay taxes in ' + state.name + '?', a: noStateTax ? 'Yes. In ' + state.name + ', Twitch streamers pay 15.3% self-employment tax plus federal income tax. There is no ' + state.name + ' state income tax.' : 'Yes. In ' + state.name + ', Twitch streamers pay 15.3% SE tax, federal income tax (10-37%), plus ' + stateRateStr + ' ' + state.name + ' state income tax.' },
                  ...
                  { q: 'Why does Twitch send two 1099 forms in ' + state.name + '?', a: 'Twitch reports subscription and ad revenue on Form 1099-MISC as royalties, while bits are reported on a separate Form 1099-NEC. Both count as self-employment income in ' + state.name + ' or any other state, and must be added together and reported on Schedule C.' },
                ].map((item, i) => (
                ...
                <div style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 4, padding: 12, fontSize: 14, color: '#8fa8c8', lineHeight: 1.6 }}>
                  <strong>Disclaimer:</strong> State tax rates are for estimation purposes only. Not affiliated with Twitch or any government agency. Consult a licensed CPA for advice.
                </div>
              </div>
            </div>
            <div style={card}>
