...
export default function GigCalculator({
  ...
  const calculate = () => {
    ...
    setResult({ net, seTax, federal, stateTax, total, quarterly: total / 4, rate: ((total / net) * 100).toFixed(1), saveRate, stateAbbr: st?.abbr ?? '?' })
  }
        ...
        <div style={{ padding: 14 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 10 }} className="form-grid">
            ...
            <div style={{ gridColumn: '1 / -1' }}>
              <label style={lbl}> Annual {platform.name} Net Income — Subs + Ads + Bits + Donations (USD)</label>
              <div style={{ position: 'relative' }}>
                <span style={{ position: 'absolute', left: 11, top: '50%', transform: 'translateY(-50%)', color: 'rgba(255,255,255,0.55)', fontWeight: 700 }}>$</span>
                <input style={{ ...inp, paddingLeft: 24 }} type="number" min="0" value={income} onChange={e => setIncome(e.target.value)} placeholder="e.g. 30,000"/>
              </div>
              <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.45)', marginTop: 6 }}>Combine amounts from both your 1099-MISC (subs, ads) and 1099-NEC (bits) — both count as self-employment income.</div>
              <div style={{display:'flex',flexWrap:'wrap',gap:'6px',marginTop:'8px',marginBottom:'4px'}}>
                {[['15000','$15k'],['30000','$30k'],['50000','$50k'],['100000','$100k']].map(([v,l])=>(
        ...
        <div style={card}>
          ...
          <div style={{ background: 'rgba(178,34,52,0.12)', borderBottom: '1px solid #fecaca', padding: '12px 20px', display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ fontSize: 13, color: '#B22234', fontWeight: 600 }}>
               Recommendation: Set aside <strong>{result.saveRate}% of every payout</strong> for taxes (approx. {fmt(parseFloat(income) * result.saveRate / 100 / 12)}/month)
            </span>
          </div>
