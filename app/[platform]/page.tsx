...
const DEADLINES = [
  { q: 'Q1 2026', due: 'April 15, 2026',   days: 45,  now: true  },
  { q: 'Q2 2026', due: 'June 16, 2026',    days: 107, now: false },
  { q: 'Q3 2026', due: 'Sept 15, 2026',    days: 198, now: false },
  { q: 'Q4 2026', due: 'Jan 15, 2027',     days: 320, now: false },
]

export function generateStaticParams() {
  return Object.keys(PLATFORMS_DATA).map(platform => ({ platform }))
}
    ...
    <div style={{ background: '#eef0f4', minHeight: '100vh' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg,#1e2d5a,#0d1b3e)', borderBottom: '1px solid rgba(255,255,255,.07)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 20px 28px' }}>
          ...
          {/* Pro tip */}
          <div style={{ paddingLeft: 16, background: 'rgba(232,184,75,.08)', border: '1px solid rgba(232,184,75,.2)', borderRadius: 6, padding: '10px 16px', maxWidth: 700 }}>
        ...
        <div>
                  ...
                  <select style={inp} value={stateCode} onChange={e => setStateCode(e.target.value)}>
                    {ALL_STATES.map(s => (
                      <option key={s.code} value={s.code}>
                        {s.name} {s.rate === 0 ? '(No State Tax)' : `(${(s.rate*100).toFixed(2)}%)`}
                      </option>
                    ))}
                  </select>
                </div>
            ...
            <div style={card}>
              ...
              <div style={{ padding: 20 }}>
                ...
                <div style={{ marginTop: 14, background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 4, padding: '10px 14px', fontSize: 12, color: '#92400e', lineHeight: 1.6 }}>
                  ⚠️ <strong>Disclaimer:</strong> Estimates for planning purposes only. Consult a licensed CPA or visit IRS.gov for official guidance.
                </div>
              </div>
            </div>
          )}
          ...
          <div style={card}>
            <div style={cardHd}>
              ...
              <span style={{ marginLeft: 'auto', fontSize: 12, color: 'rgba(255,255,255,.35)' }}>All 51 jurisdictions</span>
            </div>
            <div style={{ padding: 16, display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8 }} className="states-grid">
              {ALL_STATES.map(s => (
                <Link key={s.slug} href={`/${params.platform}/${s.slug}`} style={{ textDecoration: 'none' }}>
                  ...
                  >
                    ...
                    <div style={{ fontSize: 12, color: s.rate === 0 ? '#059669' : '#B22234', fontWeight: 600, marginTop: 2 }}>
                      {s.rate === 0 ? 'No State Tax ✓' : `State: ${(s.rate*100).toFixed(2)}%`}
                    </div>
                  </div>
                </Link>
              ))}
        ...
        <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 16 }}>
          ...
          <div style={{ background: '#f8fafc', border: '1px solid #d8dce6', borderRadius: 6, padding: 16 }}>
            ...
            <div style={{ fontSize: 12, color: '#374151', lineHeight: 1.7 }}>
              GigWiseTax.com is a free, independent tool. Not affiliated with {platform.name}, the IRS, or any government agency. No personal data stored.
            </div>
          </div>
