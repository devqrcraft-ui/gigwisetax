...
const PLATFORMS_DATA: Record<string, { name: string; emoji: string; tip: string }> = {
  ...
  'airbnb':      { name: 'Airbnb',      emoji: '🏠', tip: 'Deduct mortgage interest, utilities, and cleaning pro-rated by rental days.' },
...
const STATES_DATA: Record<string, { name: string; code: string; rate: number; flag: string; note: string }> = {
  ...
  'ohio':           { name: 'Ohio',            code: 'OH', rate: 0.04,   flag: '🏈', note: 'Ohio has a graduated income tax with rates up to 4%.' },
...
export function generateStaticParams() {
  return Object.keys(PLATFORMS_DATA).flatMap(platform =>
    Object.keys(STATES_DATA).map(state => ({ platform, state }))
  )
}

export default function PlatformStatePage({ params }: { params: { platform: string; state: string } }) {
  const platform = PLATFORMS_DATA[params.platform]
      ...
      <div style={{ background: 'linear-gradient(135deg,#1e2d5a,#0d1b3e)', borderBottom: '1px solid rgba(255,255,255,.07)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 20px 28px' }}>
          ...
          <div style={{ fontSize: 12, color: 'rgba(255,255,255,.35)', marginBottom: 12 }}>
            ...
            {' › '}
            <Link href={`/${params.platform}`} style={{ color: 'rgba(255,255,255,.35)', textDecoration: 'none' }}>{platform.name}</Link>
            {' › '}
          ...
          <p style={{ color: 'rgba(255,255,255,.55)', fontSize: 14, lineHeight: 1.8, paddingLeft: 16, maxWidth: 780, marginBottom: 14 }}>
            ...
            {state.rate > 0
              ...
              : ` and federal income tax. ${state.name} has no state income tax — a big advantage!`
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '24px 20px 48px', display: 'grid', gridTemplateColumns: '1fr 300px', gap: 24 }} className="main-grid">
        ...
        <div>
            ...
            <div style={card}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', borderBottom: '1px solid #e2e5e9' }} className="results-row">
                {[
                  ...
                  { label: `${state.code} State Tax`, val: state.rate === 0 ? '$0 ✓' : fmt(result.stateTax), sub: state.rate === 0 ? 'No state tax!' : 'Estimated', green: state.rate === 0 },
          ...
          <div style={card}>
            ...
            <div style={{ padding: 16, display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8 }} className="states-grid">
              {Object.entries(STATES_DATA).filter(([slug]) => slug !== params.state).map(([slug, s]) => (
                <Link key={slug} href={`/${params.platform}/${slug}`} style={{ textDecoration: 'none' }}>
                  <div style={{ border: '1px solid #e2e5e9', borderRadius: 6, padding: '12px 14px', background: '#fff', cursor: 'pointer', transition: 'all .15s' }}
                    onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.borderColor = '#B22234'; (e.currentTarget as HTMLDivElement).style.background = '#fff5f5' }}
                    onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.borderColor = '#e2e5e9'; (e.currentTarget as HTMLDivElement).style.background = '#fff' }}
                  >
        ...
        <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 16 }}>
          ...
          <div style={{ background: '#fff', border: '1px solid #d8dce6', borderRadius: 6, overflow: 'hidden' as const }}>
            ...
            {Object.entries(PLATFORMS_DATA).filter(([slug]) => slug !== params.platform).map(([slug, p]) => (
              <Link key={slug} href={`/${slug}/${params.state}`} style={{ textDecoration: 'none' }}>
          ...
          <div style={{ background: '#f8fafc', border: '1px solid #d8dce6', borderRadius: 6, padding: 16 }}>
            ...
            <div style={{ fontSize: 12, color: '#374151', lineHeight: 1.7 }}>
              Independent free tool. Not affiliated with {platform.name}, IRS, or any government agency. No personal data stored.
            </div>
          </div>
