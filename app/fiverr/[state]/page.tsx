...
export async function generateMetadata({ params }: { params: Promise<{ state: string }> }): Promise<Metadata> {
  ...
  return {
    title: s.rate === 0
      ...
      : `Fiverr ${s.name} Taxes 2026 — $60K Earner Owes $${(14278 + Math.round(60000 * s.rate)).toLocaleString()} Total`,
    description: `Fiverr ${s.name} 2026: $60K net = ~$8,478 SE tax + $5,800 fed. ${stateStr}. Quarterly: ~$3,570. Free calculator, no signup.`,
...
export default async function StatePage({ params }: { params: Promise<{ state: string }> }) {
  ...
  const stateNote = state.name + (noStateTax ? ' has no state income tax. Fiverr freelancers in ' + state.name + ' only pay federal income tax and the 15.3% self-employment tax.' : ' has a ' + stateRateStr + ' state income tax rate. Fiverr freelancers in ' + state.name + ' must make both federal and state quarterly estimated tax payments.')
  ...
  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: '{"@context":"https://schema.org","@type":"FAQPage","mainEntity":[{"@type":"Question","name":"Do Fiverr freelancers pay taxes?","acceptedAnswer":{"@type":"Answer","text":"Yes. Fiverr classifies freelancers as independent contractors. Fiverr does not withhold taxes. If you earn $400 or more you must pay self-employment tax (15.3%) plus federal and state income tax."}},{"@type":"Question","name":"Does Rover send a 1099?","acceptedAnswer":{"@type":"Answer","text":"Fiverr sends a 1099-K if you process $5,000 or more through the platform in a year. Even without a 1099 all income is taxable and must be reported on Schedule C."}},{"@type":"Question","name":"What can Fiverr freelancers deduct?","acceptedAnswer":{"@type":"Answer","text":"Fiverr freelancers can deduct pet supplies, dog food used for client pets, leashes, mileage driving to client homes (72.5 cents/mile in 2026), a home office if used exclusively for the business, and any training or certification costs."}}]}' }} />
        ...
        <div style={{ background: 'linear-gradient(135deg,#1e2d5a,#07111F)', borderBottom: '1px solid rgba(255,255,255,.07)' }}>
          <div style={{ maxWidth: 1200, margin: '0 auto', padding: '16px 16px 12px' }}>
            ...
            <p style={{ color: 'rgba(255,255,255,.55)', fontSize: 14, lineHeight: 1.8, paddingLeft: 16, maxWidth: 780, marginBottom: 18 }}>
              {'Free ' + platform.name + ' freelancer tax calculator for ' + state.name + ' (' + state.abbr + ').'}
          ...
          <div>
            ...
            <div style={card}>
              ...
              <div style={{ padding: 14 }}>
                ...
                {[
                  { q: 'Do Fiverr freelancers pay taxes in ' + state.name + '?', a: noStateTax ? 'Yes. In ' + state.name + ', Fiverr freelancers pay 15.3% self-employment tax plus federal income tax. There is no ' + state.name + ' state income tax.' : 'Yes. In ' + state.name + ', Fiverr freelancers pay 15.3% SE tax, federal income tax (10-37%), plus ' + stateRateStr + ' ' + state.name + ' state income tax.' },
                  ...
                  { q: 'Does Rover take out taxes in ' + state.name + '?', a: 'No. Fiverr does not withhold taxes. You are responsible for paying all SE tax, federal income tax' + (noStateTax ? '' : ', and ' + state.name + ' state income tax') + ' yourself through quarterly estimated payments.' },
                ].map((item, i) => (
                ...
                <div style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 4, padding: 12, fontSize: 14, color: '#8fa8c8', lineHeight: 1.6 }}>
                  <strong>Disclaimer:</strong> State tax rates are for estimation purposes only. Not affiliated with Fiverr or any government agency. Consult a licensed CPA for advice.
                </div>
              </div>
            </div>
            <div style={card}>
