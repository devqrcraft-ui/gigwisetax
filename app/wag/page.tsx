...
export async function generateMetadata({ params }: { params: { platform: string } }): Promise<Metadata> {
  ...
  return {
    ...
    keywords: `wag tax calculator, wag self employment tax 2026, wag quarterly taxes, wag 1099 taxes, how much taxes do wag workers pay`,
...
export default function PlatformPage({ params }: { params: { platform: string } }) {
  ...
  const deductions = DEDUCTIONS['wag' as keyof typeof DEDUCTIONS] || DEDUCTIONS.wag
  ...
  return (
    <>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: `{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"Home","item":"https://www.gigwisetax.com"},{"@type":"ListItem","position":2,"name":"${platform.name} Tax Calculator","item":"https://www.gigwisetax.com/wag"}]}` }}/>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: `{"@context":"https://schema.org","@type":"WebPage","name":"${platform.name} Tax Calculator 2026","url":"https://www.gigwisetax.com/wag","dateModified":"2026-08-24","author":{"@type":"Organization","name":"the GigWiseTax Team","url":"https://www.gigwisetax.com"},"publisher":{"@type":"Organization","name":"GigWiseTax","url":"https://www.gigwisetax.com"}}` }}/>
          ...
          <div style={{ background:'rgba(232,184,75,0.06)', border:'1px solid rgba(232,184,75,0.2)', borderRadius:8, padding:'20px 24px', margin:'16px 0' }}>
            ...
            <div style={card}>
              ...
              <div style={{ padding: 16, display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10 }} className="p-grid">
                {PLATFORMS.filter(p => p.slug !== 'wag').map(p => (
...
<div style={{ background: '#07111F', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 10, overflow: 'hidden' as const, marginBottom: 16 }}>
          ...
          {/* OWN BANNER — privatepaycheck.com */}
            ...
            {/* STATES LINKS */}
            <PlatformStatesGrid platformSlug="wag" />
